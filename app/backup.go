...
func backupNameAllowed(name string) bool {
	...
	return name == "vm/disk.raw" || name == "settings.json" || name == desktopPreferencesFilename || name == audioPreferencesFilename || name == resourcePreferencesFilename || name == storageSettingsFilename || strings.HasPrefix(name, "guest/") || strings.HasPrefix(name, "runtime/")
}
...
func writeVMArchive(dir, destination string, report backupProgress, checkpoint bool) error {
	...
	var total int64
	roots := []string{"guest", "runtime", "vm/disk.raw", "settings.json", storageSettingsFilename, desktopPreferencesFilename, audioPreferencesFilename, resourcePreferencesFilename}
	for index := 0; index < maximumGuestDisplays; index++ {
