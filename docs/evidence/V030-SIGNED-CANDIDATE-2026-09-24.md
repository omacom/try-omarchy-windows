...
[Publish run 35978943238](https://github.com/omacom/try-omarchy-windows/actions/runs/35978943238) published `v0.3.0` as Latest. The public `SHA256SUMS` digest is `0f34b70c7551839e9395030d5e3861c85178c7b5f9a4b61ed76a6e177127d51d`. Public `update-v2.json` identifies version `v0.3.0`, that manifest digest, and launcher SHA-256 `906356dc484c1d6e61f50d4fd28e42762ada06c514961ca41ef2aeef64d7e0f5`. Its release URL uses the legacy `tsouth89` redirect by design.

The public v0.2.0 launcher was downloaded from its tagged release and verified on the laptop. Its SHA-256 was `57f3b920f8384fb8eddd6715d4600b9ec11eb22a5323d1c6b377ad33cb7eaa8d`; Authenticode was `Valid`, signer `CN=Brandon South`, and FileVersion and ProductVersion were `v0.2.0`. It was copied to the stable launcher path in the disposable copy. The launch used `-dir public-update-copy`, `-winq` set to a nonexistent directory, SSH port `2253`, the source-specific pinned key, and `-start`. It did not use `-no-update` or local release URL or checksum overrides.

The public v0.2.0 launcher logged `04:24:47 starting authenticated launcher update`; the v0.3.0 stable launcher started at `04:24:48`. It did not first update the v0.1.0 guest to v0.2.0. The log contains no `guest update v0.2.0` line. The guest and runtime were updated directly to the public v0.3.0 payload. The launcher logged `04:35:39 launcher update v0.3.0 confirmed after healthy boot` and `04:35:40 guest update v0.3.0 confirmed after userspace reported ready`.

On the laptop, the installed stable launcher at `D:\TryOmarchy-v0.3.0-20260924\public-update-copy\TryOmarchy.exe` had SHA-256 `906356dc484c1d6e61f50d4fd28e42762ada06c514961ca41ef2aeef64d7e0f5`, Authenticode `Valid`, signer `CN=Brandon South`, and FileVersion and ProductVersion `v0.3.0`. The guest receipt held URL `https://github.com/omacom/try-omarchy-windows/releases/download/v0.3.0` and manifest digest `0f34b70c7551839e9395030d5e3861c85178c7b5f9a4b61ed76a6e177127d51d`. The installed QEMU SHA-256 was the r20c value `44e6e56f88fcac5567bfc3995aa5113135efe18b22cd2d0316eb30819c254447`; its command line included `virtio-vga-gl` and `-audiodev sdl`.

The first public update boot reached userspace at `04:35:38` and logged the launcher and guest update confirmations. Its first session-1 screenshot watcher hit a sharing violation while reading the concurrently-written `vm/shell.log`, so it did not capture that boot. On a public v0.3.0 stable relaunch, userspace was ready at `04:43:37` and the session-1 Windows screenshot was captured at `04:44:03`, 26 seconds later. Its in-image clock reads `4:44 AM`; it shows the Omarchy bar but a black main canvas around the CSSI dialog. That frame is `D:\TryOmarchy-v0.3.0-20260924\evidence\public-windows-1.png` and `/tmp/v030/evidence/public-windows-1.png`. A guest `grim` image at `04:45:00` showed the desktop and bar; it is `D:\TryOmarchy-v0.3.0-20260924\evidence\public-guest-grim.png` and `/tmp/v030/evidence/public-guest-grim.png`. I then powered off cleanly and relaunched the public v0.3.0 stable launcher on the same copy. Userspace was ready at `04:50:26`. The session-1 Windows screenshot was captured at `04:51:18`, 52 seconds after readiness, and its taskbar clock reads `4:51 AM`. It shows the Omarchy wallpaper and bar with the CSSI dialog over the center. No dialog button was clicked and no key was sent to it. The matching guest `grim` image was captured at `04:51:42`; its bar clock reads `Thursday 04:51`, and `hyprctl monitors -j` reported `Virtual-1`, `1366x697`, `dpmsStatus=true`.

Windows screenshot paths are `D:\TryOmarchy-v0.3.0-20260924\evidence\public-windows-2.png` and `/tmp/v030/evidence/public-windows-2.png`. Guest screenshot paths are `D:\TryOmarchy-v0.3.0-20260924\evidence\public-guest-grim-second.png` and `/tmp/v030/evidence/public-guest-grim-second.png`. The guest reached `systemd is-system-running=running` with 0 failed units, the preserved marker still matched `d7c3d98dd030ccf076e9bc8d1529ef6814c314704c610486d7256e93744e7ade`, and `omarchy-windows-audio-bridge.service` was active and enabled.
