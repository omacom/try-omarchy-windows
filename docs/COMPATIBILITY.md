...
- Public `v0.3.0` ships the bundled r20c runtime for live audio routing and free-page reporting. Public `v0.2.0` used r19. A separately installed `C:\WINQ-EMU` is user-managed and can be selected instead; older external builds may not support free-page reporting.
...
- Public `v0.3.0` uses r20c for live playback and recording device switching from host Settings and Omarchy's guest audio picker. Saved choices persist across guest reboots, and microphone access changes apply at the next VM start. Public `v0.2.0` remains a previous startup-only release. Two physical endpoints per direction and hotplug remain untested.
