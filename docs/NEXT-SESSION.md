...
[PR #164](https://github.com/omacom/try-omarchy-windows/pull/164) implements three gaps: opening native host Settings from the Omarchy launcher, a Windows battery/AC mirror exposed as BAT0/ADP0, and live WHPX RAM reclamation through WINQ-EMU r19. The AMD Windows 11 laptop passed guest Settings, a real 99% charging battery in UPower, three RAM allocation/free/reuse cycles with about 797 MiB returned after the third cycle, and a full GPU-path desktop boot on r19. Read the [exact physical record](evidence/FEATURE-GAPS-2026-09-23.md) before changing acceptance claims.

The matching r19 portable and source archives, plus `SHA256SUMS`, are attached to a **draft** `runtime-v1-r19` component release targeting the exact CI build commit `751a845`. The public `guest-build/runtime.lock.json` still pins r18. Publish and pin r19, rebuild the guest and validate the exact signed app candidate before claiming these features are shipped. A source merge alone cannot enable RAM reclamation for `v0.1.0`.
...
- [Approved Windows app bridge #160](https://github.com/omacom/try-omarchy-windows/issues/160): phase 1 is an allowlisted host launch with clear return to Omarchy; embedded windows are a separate milestone.
