...
- [Fullscreen monitor choice #168](https://github.com/omacom/try-omarchy-windows/issues/168): source candidate adds a General page target for fullscreen, uses that monitor's native resolution and restores to primary when the target is absent. Native Windows monitor enumeration and settings persistence passed on the AMD laptop. A second active monitor was not available for direct placement acceptance.
